...
const Header = () => {
  ...
  return (
    <header className="w-full h-[90px] bg-bg sticky z-50 top-[-1px] border-solid border-b-2 shadow-custom-shadow">
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} w-full h-[100dvh] bg-custom-bg`} onClick={() => { setIsOpen(!isOpen) }}>
        <div className={`${isOpen ? 'h-[450px] overflow-visible' : 'h-0 overflow-hidden'} absolute w-full bg-bg top-[88px] border-b-2 border-t-2`}
