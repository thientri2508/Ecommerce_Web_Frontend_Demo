...
const ListCategory = ({
  ...
  return (
    <div className="flex flex-wrap gap-x-16 gap-y-8 mt-8 w-[65%] items-start">
      {categoriesLv2?.map((categoryLv2: Category) => {
        return (
          <div key={categoryLv2?.id} className="w-[28%]">
            ...
            <ul className="*:text-text-muted *:cursor-pointer *:transition-all *:duration-200 text-subCategory flex flex-col gap-2 mt-3">
              {categories
